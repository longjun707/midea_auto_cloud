...
class MiedaDevice(threading.Thread):
        ...
        if attribute in self._attributes.keys():
            ...
            if self._lua_runtime is not None:
                try:
                    ...
                    MideaLogger.debug(f"LuaRuntimeError in set_attribute {nested_status}: {repr(e)}, {traceback.format_exc()}")
        ...
        if has_new:
            if self._lua_runtime is not None:
                try:
                    ...
                    MideaLogger.debug(f"LuaRuntimeError in set_attributes {nested_status}: {repr(e)}, {traceback.format_exc()}")
    ...
    async def send_command(self, cmd_type, cmd_body: bytearray):
        cmd = MessageQuestCustom(self._device_type, cmd_type, cmd_body)
        try:
            await self._build_send(cmd.serialize().hex())
                ...
                if lvalue and rvalue:
                    ...
                    if calculate:
                        ...
                        try:
                            ...
                            MideaLogger.debug(
                                f"Calculation Error: {lvalue} = {rvalue}, calculate_str1: {calculate_str1}, calculate_str2: {calculate_str2}, {traceback.format_exc()}",
                                self._device_id
                            )
            if update:
    ...
    def _device_connected(self, connected=True):
        ...
        if not connected:
            MideaLogger.info(f"Device {self._device_id} disconnected", self._device_id)
        else:
