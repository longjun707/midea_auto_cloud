...
class MideaCloud:
    ...
    async def _api_request(self, endpoint: str, data: dict, header=None, method="POST") -> dict | None:
        ...
        try:
            ...
            _LOGGER.debug(f"API request error: {e}, {traceback.format_exc()}")
...
class MeijuCloud(MideaCloud):
    ...
    async def login(self) -> bool:
        if login_id := await self._get_login_id():
            ...
            if response := await self._api_request(
                ...
                # uid 在 mdata.userInfo.uid
                user_info = response["mdata"].get("userInfo", {})
                self._uid = user_info.get("uid", "")
    ...
    # ===== SSE 实时状态推送 =====
    async def start_sse_listener(self) -> bool:
        """启动 SSE 实时状态监听"""
        if self._sse_client and self._sse_client.is_running:
            return True
        
        if not self._access_token or not self._uid:
            _LOGGER.warning("SSE: Cannot start without access_token or uid")
            return False
        
        try:
            from .sse_client import MideaSSEClient
            self._sse_client = MideaSSEClient(
            ...
            _LOGGER.error(f"SSE: Failed to start listener: {e}")
            return False
    ...
    def register_sse_callback(self, device_id: str, callback) -> None:
        """注册设备状态回调"""
        if self._sse_client:
            self._sse_client.register_device_callback(str(device_id), callback)

    def unregister_sse_callback(self, device_id: str) -> None:
        """取消注册设备状态回调"""
        if self._sse_client:
            self._sse_client.unregister_device_callback(str(device_id))

    @property
    def sse_running(self) -> bool:
        """返回 SSE 是否正在运行"""
        return self._sse_client.is_running if self._sse_client else False
    ...
    async def get_token_key(self, appliance_code: int) -> dict | None:
        """获取设备的 Token 和 Key，用于本地控制"""
        # 尝试使用美的美居的设备密钥接口
        data = {
            "applianceCode": str(appliance_code),
        }
        # 尝试不同的 API 端点
        endpoints = [
            "/v1/appliance/token/get",
            "/v1/iot/secure/getToken",
            "/v1/appliance/info/get",
        ]
        for endpoint in endpoints:
            try:
                if response := await self._api_request(
                    ...
                    token = response.get("token") or response.get("tokenlist", [{}])[0].get("token")
                    key = response.get("key") or response.get("tokenlist", [{}])[0].get("key")
                    if token and key:
                        MideaLogger.info(f"Got token/key for device {appliance_code} from {endpoint}")
                        return {"token": token, "key": key}
            except Exception as e:
                MideaLogger.debug(f"Failed to get token from {endpoint}: {e}")
                continue
        return None
    ...
    async def download_lua(
        ...
        # 检查本地是否已有匹配的 lua 文件
        device_type_hex = "%02X" % device_type
        pattern = f"{path}/T_*_{device_type_hex}_*.lua"
        existing_files = await asyncio.to_thread(glob.glob, pattern)
        if existing_files:
            MideaLogger.info(f"Found existing lua file: {existing_files[0]}, skip download")
        ...
        if response := await self._api_request(
            ...
            res = await self._session.get(response["url"], timeout=ClientTimeout(total=60))
            if res.status == 200:
                lua = await res.text()
                if lua:
    ...
    async def download_plugin(
        ...
        if response := await self._api_request(
            ...
            # 检查本地是否已有匹配的 plugin 文件
            plugin_pattern = f"{path}/T0x{device_type:02X}_*.zip"
            existing_plugins = await asyncio.to_thread(glob.glob, plugin_pattern)
            if existing_plugins:
                MideaLogger.info(f"Found existing plugin file: {existing_plugins[0]}, skip download")
                return existing_plugins[0]
            ...
            try:
                ...
                res = await self._session.get(zip_url, timeout=ClientTimeout(total=120))
                if res.status == 200:
...
class MSmartHomeCloud(MideaCloud):
    ...
    async def _api_request(self, endpoint: str, data: dict, header=None, method="POST") -> dict | None:
        ...
        return await super()._api_request(endpoint, data, header, method=method)
