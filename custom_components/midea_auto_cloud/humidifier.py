...
class MideaHumidifierEntity(MideaEntity, HumidifierEntity):
    ...
    def __init__(self, coordinator, device, manufacturer, rationale, entity_key, config):
        ...
        self._attr_available_modes = list((self._config.get("modes") or {}).keys())
    ...
    def is_on(self):
        ...
        if power_key:
            return self._get_status_on_off(power_key)
        return False
    ...
    async def async_turn_on(self, **kwargs):
        ...
        power_key = self._config.get("power")
        if power_key:
            await self.async_set_attribute(power_key, self._rationale[int(True)])

    async def async_turn_off(self, **kwargs):
        """Turn the humidifier off."""
        power_key = self._config.get("power")
        if power_key:
            # 发送两次关闭命令，避免美的加湿器挂机启动风干湿帘，噪音过大
            await self.async_set_attribute(power_key, self._rationale[int(False)])
            await self.async_set_attribute(power_key, self._rationale[int(False)])
    ...
    async def async_set_humidity(self, humidity: int):
        ...
        target_humidity_key = self._config.get("target_humidity")
        if target_humidity_key:
            await self.async_set_attribute(target_humidity_key, humidity)
