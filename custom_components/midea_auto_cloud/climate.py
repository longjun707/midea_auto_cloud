...
class MideaClimateEntity(MideaEntity, ClimateEntity):
    ...
    def target_temperature(self):
        if self._is_central_ac:
            ...
            if run_mode == "2":  # 制冷模式
                ...
                return self._get_nested_value("heat_temp_set")
            elif run_mode == "4":  # 自动模式 - 使用 cool_temp_set 作为显示
                return self._get_nested_value("cool_temp_set")
            elif run_mode == "5":  # 除湿模式 - 使用 cool_temp_set 作为显示
                return self._get_nested_value("cool_temp_set")
            # run_mode == "0" (关机) 或 "1" (送风) 返回 None
            return None
        else:
    ...
    @property
    def min_temp(self):
        if self._key_min_temp is None:
            return 16.0
        if isinstance(self._key_min_temp, str):
            val = self.device_attributes.get(self._key_min_temp)
            return float(val) if val is not None else 16.0
        return float(self._key_min_temp)

    @property
    def max_temp(self):
        if self._key_max_temp is None:
            return 30.0
        if isinstance(self._key_max_temp, str):
            val = self.device_attributes.get(self._key_max_temp)
            return float(val) if val is not None else 30.0
        return float(self._key_max_temp)

    @property
    def min_humidity(self):
        if self._key_min_humidity is None:
            return 45.0
        if isinstance(self._key_min_humidity, str):
            val = self.device_attributes.get(self._key_min_humidity)
            return float(val) if val is not None else 45.0
        return float(self._key_min_humidity)

    @property
    def max_humidity(self):
        if self._key_max_humidity is None:
            return 65.0
        if isinstance(self._key_max_humidity, str):
            val = self.device_attributes.get(self._key_max_humidity)
            return float(val) if val is not None else 65.0
    ...
    @property
    def hvac_modes(self):
        if self._key_hvac_modes is None:
            return []
    ...
    def _get_status_on_off(self, key):
        ...
        Overrides parent to handle broader truthy values common in climate devices.
        """
        if key is None:
            return False
        ...
        if isinstance(value, bool):
            return value
        # Try rationale-based matching first (consistent with parent)
        try:
            return bool(self._rationale.index(value))
        except ValueError:
            pass
        # Fallback: common truthy representations
        if isinstance(value, int) or (isinstance(value, str) and value in ('0', '1')):
            return int(value) != 0
    ...
    async def _async_set_status_on_off(self, attribute_key: str | None, turn_on: bool):
        ...
        if turn_on and self._key_pre_mode is not None:
            pre_mode_val = self._get_nested_value(self._key_pre_mode)
            if pre_mode_val is not None:
                new_status[self._key_pre_mode] = pre_mode_val
        await self.async_set_attributes(new_status)
